...
$(document).ready(function(){
  ...
  //   	$("#suc").fadeOut();
	...
	if($(".reg_title").attr("data") == "profession"){
		$("input[name='realname']").prev().html("");
		$("input[name='realname']").prev().append("<span>*</span>姓名：");
	}

	if($(".report").attr("id") == "专家用户"){
		$(".report #doc").children(".reportbtn").css("display","");
		$(".report #report").children(".reportbtn").css("display","");
	}
	else{
		$(".report #doc").children(".reportbtn").css("display","none");
		$(".report #report").children(".reportbtn").css("display","none");
	}
	...
	$(".udetail .setinput").change(function(){
 	...
 	// checkup5上传图片
 	$(".cfunc .cliupload").click(function(){
 		var tt = $(this).attr("id");
 		$(".cfunc ."+tt).show();
 	});

 	$(".wdelete").click(function(){
 		var ysr = $(this).next(".trimg").attr('src');
 		var sr = ysr.substring(6);
 		var posi = $(this).parents(".imgposi").attr("id");
 		$.post("/t/deleteimg",
 				{sr1:sr,posi1:posi,},
 				function(data){
 					if(data == "success"){
 						$("img[src='"+ysr+"']").parents(".yshow").remove();
 						$("li[data='"+ysr+"']").remove();
 					}	
 					else{
 						alert("图片删除失败！"+data);
	...
	$(".cinfoi").on("change",function() {
		var o = $(this).parents(".dlisf").prevAll(".title").attr("id");
		var r = $(this).parents(".dlisf").prevAll(".title").text();
		var addspan = $("#"+o+"_"+r);
		...
		if(ids != null){
			...
			var ls = $(this).parents().prevAll(".title").parents(".locationcontentmark").attr("data");
			// $("#dinfotip #"+pids+" h2").html("【 "+ls+" 】");
			// $("#dinfotip #"+pids+" h2").show();
			$("#dinfotip #"+pids+" h3").html("【 "+ls+" 】"+"--"+"【 "+s+" 】");
	...
	$("#quantity .cinfoi").on("change",function(){
		var pids = $(this).parents(".dlisf").prevAll(".title").attr("id");
		var cop = $(this).parents(".item").parent().attr("id");
		var lia = cop.substr(4,1);
		var qs = cop.substr(0,3);
		var qq = $("#"+cop+" #degree").children("input[name='m"+qs+"nl"+lia+"']:checked");
		var copval = $(this).val();
		switch(copval){
			case "0":
				var mcop = "个别";
		...
		if(qq.size() == 0){
			$("#"+cop+" #degree").children("input[id='qw']").attr("checked","checked");
			var snr = "轻微"
			$("#dinfotip #"+pids+" #i"+cop+" .nl").html(mcop+snr);
		}
		else{
		var nr = $("#dinfotip #"+pids+" #i"+cop+" .nl").html();
		var snr = nr.substring(2,4);
		$("#dinfotip #"+pids+" #i"+cop+" .nl").html(mcop+snr);}
	});
	...
	$(".mhtitle .func").click(function(){
		$(".mhtitle .func").removeClass("hover");
		$(this).addClass("hover");
		
		$(".modal-body .cfunc").hide();
		$("#" + $(this).attr("data")).show();
	});
	...
	$(".wzs").click(function(){
		...
		$("#dinfotip #d"+$(this).parents(".oitem").children(".item").attr("id")).css("display","none");
		$("#dinfotip #p"+$(this).parents(".oitem").children(".item").attr("id")).css("display","none");
		
		var cp = $("#dinfotip #p"+$(this).parents(".oitem").children(".item").attr("id")).parents(".xtip").children("div:visible");
		if(cp.length == "0"){
			$("#dinfotip #p"+$(this).parents(".oitem").children(".item").attr("id")).prevAll("h3").css("display","none");
		}
		$("#dinfotip #d"+$(this).parents(".oitem").children(".item").attr("id")+" #parent").nextAll().remove();
		
		var zid = $(this).parents(".dlisf").prevAll(".title").attr("id");
		var zhtml = $(this).parents(".dlisf").prevAll(".title").html();
		var ne = $(this).parents(".dlisf").nextAll(".dlisf").children(".oitem").children(".item").children(".fieldsetborder").children("input[class='cinfoi']:checked");
		var pr = $(this).parents(".dlisf").prevAll(".dlisf").children(".oitem").children(".item").children(".fieldsetborder").children("input[class='cinfoi']:checked");
		if ((ne.size() == "0") &&(pr.size()==  "0")){
		$("#"+zid+"_"+zhtml).removeClass("hover");
		$("#"+zid+"_"+zhtml).children("span").remove();}
...
$(".addsearchMap").click(function(){
	...
	$(".uinfo .sbtn").mouseover(function(){
		$(this).next(".sbtns").css("z-index","2");
		$(this).prev(".sinptxt").css("z-index","-2");
	});
	
	$(".uinfo .sbtns").mouseleave(function(){
		$(this).css("z-index","-2");
		$(this).prev(".sbtn").prev(".sinptxt").css("z-index","2");
	});
	...
	$(".uinfo .inptxt").blur(function(){
		...
		var arr = ["pass", "rpass", "realname", "idnum", "major", "depart","title", "address","zipcode" ,"email"];
		var t = o.attr("data");
		var v = o.val();
		...
		switch(in_array(t,arr)){
			...
			// 		if(v==""){
			// 			$("#phonemsg").text("手机号码不可为空");
			// 			$("#phonemsg").show();
			// 			$(".tipphone").show();
			// 			$(".sucphone").hide();
			// 		}
			// 		else if(v.length!=11){
			// 			$("#phonemsg").text("手机号码长度为11位");
			// 			$("#phonemsg").show();
			// 			$(".tipphone").show();
			// 			$(".sucphone").hide();
			// 		}	
			// 		else if(!phone.test(v)){
			// 			$("#phonemsg").text("手机号码不符合规则");
			// 			$("#phonemsg").show();
			// 			$(".tipphone").show();
			// 			$(".sucphone").hide();
			// 		}							
	...
	$("#userphone").blur(function(){
		$.ajax({
			url:'/n/forgotphone?userphone='+$("#userphone").val()+'&uname='+$("#name1").val(),
			type:'get',
			dateType:'json',
			success:function(data){
				$("#tel").text(data.msg);
	...
	$("#button").click(function(){
		$.ajax({
			url:'/n/button?u_tel='+$("#userphone").val(),
			type:'get',
			dateType:'json',
			success:function(data){
	...
	$("#tcod").blur(function(){
		$.ajax({
			url:'/n/tcode?tcod='+$("#tcod").val()+'&tel='+$("#userphone").val(),
			type:'get',
			dateType:'json',
			success:function(data){
				$("#tcod1").text(data.msg);
	...
	$("#submit1").click(function(){
		...
		if(($("#agreement").attr("checked")!="checked")){
			// alert("请选择服务条款");
			$("#serveritem").text("请选择服务条款");
			$("#serveritem").show();
			return false;
		}
		if(($("#tcod1").text()=="success") &&  ($("#tel").text()=="success"))  {
			return true;			
		}
		else{			
			alert("请按照提示认真填写信息");
	...
	$(".compl_btn").click(function(){
		...
		if(($("#cardnum").text()!="sucess") && ($(".uinfo #idnum").val() != "")){
			// alert("身份证号未按规则填写");
			$("#cardnum").text("身份证号未按规则填写");
			$("#cardnum").show();
			return false;
		}
		if($("#majormsg").text()!="sucess"){
			// alert("专业未按规则填写");
			$("#majormsg").text("专业不能为空");
			$("#majormsg").show();
			return false;
		}
		if($("#departmsg").text()!="sucess"){
			// alert("单位未按规则填写");
			$("#departmsg").text("单位不能为空");
			$("#departmsg").show();
			return false;
		}
		if($("#titlemsg").text()!="sucess"){
			// alert("职称未按规则填写");
			$("#titlemsg").text("职称不能为空");
			$("#titlemsg").show();
			return false;
		}
		if(($("#addressmsg").text()!="sucess") && ($(".uinfo #address").val() != "")){
			// alert("联系地址未按规则填写");
			$("#addressmsg").text("联系地址未按规则填写");
			$("#addressmsg").show();
			return false;
		}
	    if(($("#zipcodemsg").text()!="sucess") && ($(".uinfo #zipcode").val() != "")){
			// alert("邮政编码未按规则填写");
			$("#zipcodemsg").text("邮政编码未按规则填写");
			$("#zipcodemsg").show();
			return false;
		}
		if($("#phonemsg").text()!="sucess") {
			// alert("手机号码未按规则填写");
			
			$("#phonemsg").show();
			return false;
		}
		if($("#emailmsg").text()!="sucess"){
			// alert("保密邮箱未按规则填写");
			$("#emailmsg").text("保密邮箱不能为空");
			$("#emailmsg").show();
			return false;
		}
		if(($("#majormsg").text()=="sucess") && ($("#phonemsg").text()=="sucess") && ($("#departmsg").text()=="sucess") && ($("#titlemsg").text()=="sucess") && ($("#emailmsg").text()=="sucess")){
			return true;			
		}
		else{
			alert("请按照提示认真填写信息");
...
//checkup5点击切换
	...
	$(".udetail input[name='profession']").blur(function(){
		var v = $(this).val();
		if(v == "")
		{
			$("#profession").text("专业不能为空");
			$("#profession").show();
		}
		else
		{
			$("#profession").text("sucess");
			$("#profession").hide();
		}
	});

	$(".udetail input[name='danwei']").blur(function(){
		var v = $(this).val();
		if(v == "")
		{
			$("#danwei").text("单位不能为空");
	...
	$(".udetail input[name='title']").blur(function(){
		var v = $(this).val();
		if(v == "")
		{
			$("#title").text("职称不能为空");
	...
	// 下拉框
	$(".udetail select[name='profession']").blur(function(){
		var n = $(this).children('option:selected').val();
		if(n == "")
		{
			$("#profession").text("专业不能为空");
	...
	$(".udetail select[name='danwei']").blur(function(){
		var n = $(this).children('option:selected').val();
		if(n == "")
		{
			$("#danwei").text("单位不能为空");
	...
	$(".udetail select[name='title']").blur(function(){
		var n = $(this).children('option:selected').val();
		if(n == "")
		{
			$("#title").text("职称不能为空");
	...
	$(".btn input[name='imgbtn']").click(function(){
		if(($("#uniname").text()=="sucess") || ($("#jyzipcode").text()=="sucess") || ($("#jytelnum").text()=="sucess") || ($("#jyuseridcard").text()=="sucess") || ($("#jyemail").text()=="sucess") || ($("#profession").text()=="sucess") || ($("#danwei").text()=="sucess") || ($("#title").text()=="sucess")){
			return true;
		}
...
function exportXls()
{
	...
	location.href = "/t/countExportXls?qstring1="+qstring;
	...
	for(var i = 1;i<d.length-1;i++)
	{
	 ...
	 $.post("/t/countAjax",
        ...
        if(data.length>0)
        {
        	...
        	{
        		$("#datatable").append("<tr><td><input type=\"checkbox\" name='checkbox1' value='"+data1[i][0]+"' class=\"bgcbox\" /></td>");
...
this.imagePreview = function(){	
	$(".preview").hover(function(e){
		...
		$(this).parent(".yshow").append("<div id='preview' style='z-index:1;'><img src='"+ this.name +"' alt='Image preview' class='iini'/>"+ c +"</div>");
		$(this).parent(".yshow").show();
		var screenImage = $(".iini");
		...
		{
			screenImage.hide();
			// screenImage.css({"width":imageWidth, "height":imageHeight});
		} 
		else if(imageWidth>500 && imageHeight<1000)
		{
			var i = imageWidth/500;
			var himageHeight = imageHeight/i;
			screenImage.css({"width":500, "height":himageHeight});
		}
		else if(imageWidth<500 && imageHeight>1000)
		{
			var i = imageHeight/400;
			var himageWidth = imageWidth/i;
			screenImage.css({"width":himageWidth, "height":400});
		}
		$("#preview")
			.css("top",(e.clientY-260) + "px")
			.css("left",(e.clientX-260) + "px")
			.fadeIn("fast");						
    },
	function(){
		this.title = this.t;	
		$(".yshow #preview").remove();
    });	
