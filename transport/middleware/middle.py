...
class SetRemoteAddrFromForwardedFor(object):
    def process_request(self, request):
    		path = request.path
    		context_dict = {}
		if not path.startswith("/t/adLogVal") and not path.startswith("/t/modUserPos") and path != "/t/logopdf" and path != "/t/pdfdataReplace" and path !="" and path != "/t/" and path != "/t/login_va/" and path !="/t/register" and not path.startswith("/t/user_name") and not path.startswith("/t/get_check_code_image") and not path.startswith('/t/authcode') and not path.startswith("/t/register") and not path.startswith("/admin/") and not path.startswith("/t/test"):
			#print path	
